...
class AndroidUtilsTests(unittest.TestCase):
    ...
    def test_push_pull_file(self):
        ...
        self.au.push_file('/storage/sdcard1/PRIVATE/foo.txt', myFile)
        myFile=self.au.pull_file('/storage/sdcard1/PRIVATE/foo.txt')
        #logger.debug('Pulled myFile as base64: %s' % (myFile, ))
        self.assertRegexpMatches(myFile, 'VGhpcyBpcyBteUZpbGUgYXMgYmFzZTY0')
        myFile=self.au.pull_file('/storage/sdcard1/PRIVATE/foo.txt', decode=True)
        ...
        myFile=self.au.push_file('/storage/sdcard1/PRIVATE/foo.txt', myFile, encode=True)
        myFile=self.au.pull_file('/storage/sdcard1/PRIVATE/foo.txt')
        #logger.debug('Pulled myFile as base64: %s' % (myFile, ))
